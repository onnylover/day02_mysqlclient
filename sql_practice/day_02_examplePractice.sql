...
-- 쿼리 2개로 풀어도 됨
-- select gender="M" , gender="F" from employees;
select gender, count(*) from employees where gender in (select gender from employees where gender in ("M","F")) group by gender;
...
select count(*) from salaries where salary > 120000 and to_date = "9999-01-01" order by salary desc;
...
select title, from_date from titles where emp_no="13250" order by from_date;
